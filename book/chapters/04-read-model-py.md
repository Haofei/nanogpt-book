...
前面三章已经建立了运行经验、项目地图和 GPT 架构图。本章开始精读源码，只关注模型本身：给定 `B x T` 的 token id，`model.py` 如何算出 `B x T x vocab_size` 的 logits。下一章会接着看 `train.py` 如何调用这个模型并更新参数。
...
真正读懂这一章，不是背下类名，而是能从 `B x T` 的 token id 一路追踪到 `B x T x vocab_size` 的 logits，并解释每一步为什么存在。下一章会把这个模型放进训练脚本里，看 batch、loss、optimizer、checkpoint 如何围绕它工作。
