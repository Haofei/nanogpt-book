...
前面 00a-00c 已经让你完成了三件事：装好环境、理解最小 PyTorch 概念、跑通一次 Shakespeare 字符级训练。本章开始回头看整个项目地图，解释你刚刚运行过的 `prepare.py`、`train.py`、`sample.py` 分别在系统里扮演什么角色。
...
nanoGPT 是一个小型 GPT 训练系统。它的核心价值是让读者在一个仓库中看到数据、模型、训练、生成四条链路如何闭合。下一章会补齐阅读模型源码前必须知道的 Transformer 基础；等这些概念就位后，第 4 章会把它们逐一对应到 `model.py`。
