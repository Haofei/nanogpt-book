...
这是主线章节的收束。前面你已经跑通过 Shakespeare 案例、读过模型和训练源码、理解了数据、采样、性能和实验设计。本章讨论下一步：如果你要把 nanoGPT 变成自己的小实验平台，应该先扩哪里，哪些改动会破坏 checkpoint 或 tokenizer 兼容性。
...
深度学习项目里，很多文件之间存在隐含契约。

`prepare.py` 决定 `vocab_size` 和 token id 范围。`model.py` 的 embedding 和 `lm_head` 必须匹配这个 `vocab_size`。`train.py` 保存 checkpoint 时会记录 `model_args`。`sample.py` 加载 checkpoint 时会根据这些参数重建模型。

所以修改 tokenizer、`vocab_size`、`block_size`、参数名或参数 shape，都不是孤立改动。它们可能同时影响训练、恢复、采样和 GPT-2 权重加载。

一个保守原则是：凡是会改变 checkpoint 结构的改动，都应该新增配置或新输出目录，不要覆盖旧实验。
...
扩展 nanoGPT 的关键是控制复杂度。先扩数据和配置，再扩评估和日志，最后再改模型结构。每次改动都要明确它影响的是数据、模型、训练、采样还是 checkpoint。到这里，本书的主线闭环完成：从第一次运行，到理解源码，再到设计和扩展自己的实验。
