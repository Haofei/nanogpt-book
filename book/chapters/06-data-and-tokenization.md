...
## 6.2 读者需要知道的 tokenizer 背景

Tokenizer 不是简单的“压缩文本”。它定义了模型世界里的基本符号。

字符级 tokenizer 的优点是不会遇到未知词，任何文本都能按字符拆开。缺点是序列变长，例如一个英文单词可能需要 5 个字符 token。序列越长，模型要做的预测步数越多，attention 也更贵。

BPE tokenizer 会把常见片段合并成 token。例如常见单词或词片段可能只占一个 token。它能缩短序列，但也带来一个事实：token 不一定等于完整单词，空格、大小写、标点都可能影响编码结果。

所以当读者比较两个模型时，要先问：它们用的是同一个 tokenizer 吗？如果不是，loss、上下文长度和生成行为都不能简单直接比较。

## 6.3 Shakespeare 字符级数据
...
训练数据里保留什么，模型就会学习什么。换行、空格、重复段落、乱码、HTML 标记、版权声明和日志前缀都会进入 token 序列。

对于小数据实验，数据清洗尤其重要。少量脏数据可能在生成中反复出现。建议至少检查：
...
不要把清洗看成和模型无关的前置工作。对语言模型来说，数据分布就是训练目标的一部分。
